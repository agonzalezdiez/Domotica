...
import gcoder
from printrun_utils import install_locale, decode_utf8, setup_logging
...
class printcore():
    ...
    def _listen(self):
        ...
        while self._listen_can_continue():
            line = self._readline()
            #print "MSG: -%s-" % line
            if(line.strip()=='ok' or line.strip()=='echo:Hardcoded Default Settings Loaded'):
                self.ready = True
            if line is None:
                break
